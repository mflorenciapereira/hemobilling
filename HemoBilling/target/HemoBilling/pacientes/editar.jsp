...
<script type="text/javascript">	
	...
	var errorEditandoPaciente = function errorEditandoPaciente(json)
	{
		showMessage( '<spring:message code="general.errorContactoController"/>' , "errorMessageDivID" );
	};
	
	var pacienteEditado = function pacienteEditado(json)
	{
	...
	function ocultarObrasSocialesAdheridas()
	{
		...
		$('#cantObrasSocialesAdheridasID').val( index );
	...
	$(document).ready(function()
	{
		...
		//Ajax Forms
		ajaxForm( 'editarPacienteForm' , pacienteEditado , null , errorEditandoPaciente );
		...
		setTimeout(function(){cargarObrasSociales();},0);
...
<script type="text/javascript">
	...
	{		
		//Cargo el nombre de la OS Actual
		$('#obraSocialActual\\.nombre').val( '${nuevoPacienteDTO.obraSocialActual.codigo}' + " - " + '${nuevoPacienteDTO.obraSocialActual.nombre}' );
		
		//Cargo las OS adheridas
		var oss = "";
	...
	$(function() 
	{
        index = ${fn:length(nuevoPacienteDTO.obrasSocialesAdheridasAuto)};
        
        var j;
        for(j=0;j<index;j++){
        	autocompletar('obrasSocialesAdheridasAuto' + j);
        }
...
<div class="center_content">
		...
		<tr>
			<td> <h2>Editar datos de Paciente</h2>	 </td>
	...
	<form:form cssClass="niceform" method="post" name="editarPacienteForm" id="editarPacienteForm" action="doEditarPaciente.htm" modelAttribute="nuevoPacienteDTO">
		...
		<fieldset>
			...
			<form:hidden path="cantObrasSocialesAdheridas" id="cantObrasSocialesAdheridasID"/>
			...
			<dl><dt></dt></dl>
			<dl class="submit">
				<dt><input type="button" name="submit" id="submit" value="Modificar" onclick="validaryComitearForm()"/></dt>
		...
		<div id="obrasSocialesAdheridas" style="display: none;" class="jqmWindow obrasSocialesAdheridas">
			...
			<table id="obrasSocialesAdicionalesAdheridas">
				<tbody>
					...
					<c:forEach items="${nuevoPacienteDTO.obrasSocialesAdheridasAuto}" varStatus="i">
					<tr>
						<td valign="middle" id="obrasSocialesAdheridasAuto${i.index}.indice"><c:out value="${i.count}"></c:out></td>
						<td><form:input id="obrasSocialesAdheridasAuto${i.index}.codigo" path="obrasSocialesAdheridasAuto[${i.index}].codigo" size="10" /></td>
						<td><form:input id="obrasSocialesAdheridasAuto${i.index}.nombre" path="obrasSocialesAdheridasAuto[${i.index}].nombre" size="70" /></td>
						<td><a id="obrasSocialesAdheridasAuto${i.index}.eliminar" href="#" onclick="eliminarOSAdherida(${i.index})">eliminar</a></td>
					</tr>
					</c:forEach>
					<tr></tr>
	...
	<div id="getDatosLaboratorioID" style="display:none;" class="jqmWindow">
		<form:form  method="post" name="getDatosLaboratorioForm" id="getDatosLaboratorioForm" cssClass="niceform" 
			...
			<dl>
				<dt>
					<label for="idSistemaLabo">C&oacute;digo de Paciente: </label>
				</dt>
				<dd>
					<table>
