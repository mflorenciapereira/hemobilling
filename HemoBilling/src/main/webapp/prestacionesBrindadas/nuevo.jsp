...
function validaryComitearForm() {
	...
	if ($('#idPacienteID').val() == "") {
		error = true;
		showMessage("Debe ingresar los datos de un paciente.", "idPacienteErrorID");
	}
	...
	if ($('#codPrestacionID').val() == "") {
		error = true;
		showMessage("Debe ingresar los datos de una prestaci&oacute;n.", "codPrestacionErrorID");
	}
...
var agregado = function agregado(json) {
	...
	$(document).ready(
			function() {
				...
				<!-- prestaciones -->
				mapaPrestaciones =${prestacionesPosiblesString};
				
				$('#codPrestacionID').change(function() {
					var descripcion='';
					descripcion=mapaPrestaciones[$('#codPrestacionID').val()];
					  if(descripcion!=undefined){
						  $('#nombrePrestacionID').val(descripcion.desc);
					  }else{
						  
						  $('#nombrePrestacionID').val('');
						  $('#codPrestacionID').val('');
					  }
					  
					});
							
				var prestacionesListSource = [];
				$.each(mapaPrestaciones, function(key,value) { 
					prestacionesListSource.push( value );
				});

				$( "#nombrePrestacionID" ).autocomplete({
					minLength: 0,
					source: prestacionesListSource,
					select: function( event, ui ) {
						$( "#nombrePrestacionID" ).val( ui.item.desc );
						$( "#codPrestacionID" ).val( ui.item.value );
				...
				mapaPacientes =${pacientesPosiblesString};
				
				
				$('#nroHCPacienteID').change(function() {
					var descripcion='';
					descripcion=mapaPacientes[$('#nroHCPacienteID').val()];
					  if(descripcion!=undefined){
						  $('#nombrePacienteID').val(descripcion.desc);
						  $('#idPacienteID').val(descripcion.id);
					  }else{
				...
				var pacientesListSource = [];
				$.each(mapaPacientes, function(key,value) { 
					pacientesListSource.push( value );
				});

				$( "#nombrePacienteID" ).autocomplete({
					minLength: 4,
					source: pacientesListSource,
					select: function( event, ui ) {
						$( "#nombrePacienteID" ).val( ui.item.desc );
						$( "#nroHCPacienteID" ).val( ui.item.value );
						$( "#idPacienteID" ).val( ui.item.id );
...
<div class="center_content">
	...
	<form:form cssClass="niceform" method="post" name="nuevoForm"
		id="nuevoForm" action="doNuevaPrestacionBrindada.htm"
		modelAttribute="nuevaprestacionbrindadadto">
		...
		<fieldset>
			...
			<dl>
				...
				<dd>
					<table>
						<tr>
							<td><form:input path="nroHCPaciente" size="10"
			...
			<dl>
				...
				<dd>
					<table>
						<tr>
							<td><form:textarea path="nombrePaciente" cols="50" rows="6" id="nombrePacienteID" /></td>
						</tr>
						<tr>
			...
			<fieldset>	
				...
				<dd> 
					<table>
						<tr>
							<td><form:input path="codPrestacion" size="10"
				...
				<dd>
					<table>
						<tr>
							<td><form:textarea path="nombrePrestacion" cols="50" rows="6" 
			...
			<dl>
				<dt>
					<label for="precioManual">Precio Unitario Manual (*):</label>
				</dt>
				<dd>
					<table>
						<tr>
