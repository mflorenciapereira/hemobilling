...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		this.selectedContainer = $('<div id="selectedContainter" class="ui-widget-content list-container selected"></div>').appendTo(this.container);
		this.availableContainer = $('<div id="availableContainer" class="ui-widget-content list-container available"></div>').appendTo(this.container);
		...
		this.availableActions.find('input').width('570px');
    ...
    **************************************/
	...
	_refreshDividerLocation: function() {
		this.selectedContainer.width("700px");
		this.availableContainer.width("700px");
...
$.extend($.ui.multiselect, {
	...
	locale: {
		...
		itemsCount:'#{count} obras sociales seleccionadas',
		itemsTotal:'#{count} obras sociales total',
		busy:'espere por favor...',
		errorDataFormat:"No pueden agregarse las opciones, formato desconocido",
		errorInsertNode:"No fue posible agregar el item:\n\n\t[#{key}] => #{value}\n\n.",
		errorReadonly:"La opción #{option} es sólo lectura",
		errorRequest:"Problema con la llamada remota. (Tipo: #{status})"
	},
	constants: {
		MESSAGE_WARNING: 0,
		MESSAGE_EXCEPTION: 1,
