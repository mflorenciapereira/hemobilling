...
function Keypad() {
	//codigo agregado para BEE
	this.firstTimeRandom = true;
	this.layoutRandomised = null;
	//fin codigo agregado para BEE
	this.BS = '\x08'; // Backspace
	this.DEL = '\x7F'; // Delete
	this.EN = '\x0D'; // Enter
	this.CL = 'x0F' //Agregado 
	...
	this.regional[''] = { // Default regional settings
		...
		buttonStatus: 'Utilice el Teclado Virtual para ingresar su clave de manera segura.', // Status text for trigger button
		closeText: 'Cerrar', // Display text for close link 
		closeStatus: 'Cerrar el Teclado Virtual y borrar clave', // Status text for close link
		clearText: 'Limpiar', // Display text for clear link
		clearStatus: 'Borrar todo el texto', // Status text for clear link
		backText: 'Volver', // Display text for back link
		backStatus: 'Borrar el ultimo caracter', // Status text for back link
		enterText: 'Aceptar', // Display text for carriage return
		enterStatus: 'Proceder', // Status text for carriage return
		shiftText: 'Shift', // Display text for shift link
		shiftStatus: 'Cambiar entre Mayusculas/Minusculas', // Status text for shift link
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		layout: this.qwertyAll, // Layout of keys
		...
		//codigo agregado para BEE
		randomiseAgain: true //Cuando sea multiple input, poder evitar que mezcle las teclas cuando cambie el target
		//fin codigo agregado para BEE
	};
...
$.extend(Keypad.prototype, {
	...
	ALFA: ['1234567890' + EN, 'qwertyuiop' + BK, 'asdfghjkl'+ENIE+ SH, CR+'zxcvbnm'], //Configuracion de teclado agregada
	/* Standard US keyboard layout. */
	NUM: ['123' + EN, '456' + BK, '789' +CL , SP+'0'+SP+CR],
	qwertySpecial: ['!@#$%^&*()_=', '`~[]{}<>\\|/'],
	qwertyAll: ['1234567890' + CL, '!@#$%^&*()_=' + BK, '`~[]{}<>\\|/' + EN, 'qwertyuiop\'"' + CR, 'asdfghjkl;:', 'zxcvbnm,.?' + SH, SB],
	...
	_destroyKeypad: function(target) {
		...
		this.firstTimeRandom = true; //codigo agregado para BEE
	...
	/* Establece que se vuelvan a mezclar las teclas.
	   @param  target  (element) the target text field */
	_resetKeypad: function(target) {
		this.firstTimeRandom = true; //codigo agregado para BEE
	},
	...
	_showKeypad: function(field) {
		...
		var anchoTeclado, anchoTeclado, cotaSupWidth, cotaInfHeight, cotaSupHeight;
		anchoTeclado = (20 * screen.width)/100;  
		altoTeclado = (20 * screen.height)/100;
		cotaSupWidth = (43 * screen.width)/100; //las cotas son para que el teclado no tape el formulario
		cotaSupHeight = (35 * screen.height)/100;
		$.keypad._hideKeypad(null, '');
		$.keypad._lastField = field;
		$.keypad._pos = [(Math.round(Math.random()*(screen.width - anchoTeclado))),
								Math.round(Math.random()*(screen.height -altoTeclado))]; //[width, heigth]
		if($.keypad._pos[0] < cotaSupWidth && $.keypad._pos[1] < cotaSupHeight){
			$.keypad._pos[0] += (cotaSupWidth - $.keypad._pos[0]) + Math.abs((Math.round(Math.random()*50)));
		}// metodo que hace(trata por ahora) de que el teclado no tape los inputs del formulario;
		var isFixed = false;
	...
	_checkExternalClick: function(event) {
		...
		if (!target.parents().andSelf().is('.' + $.keypad._mainDivClass) &&
			...
			//$.keypad._hideKeypad(); -> Editado -> previene que se cierre el teclado cuando se clickea fuera de este en la pagina
		}
	},
	...
	_closeValue: function(inst) { //Agregado 
		this._notifyKeypress(inst, $.keypad.CL);
	},
	
	_enterValue: function(inst){ //Agregado 
		this._notifyKeypress(inst, $.keypad.EN); // Avisa que se toco la tecla(para que pueda haber una reaccion)
	},
	...
	_generateHTML: function(inst) {
		...
		html.find('button').mousedown(function() { $(this).addClass(activeClasses); }).
			...
			filter('.keypad-close').click(function() {
				$.keypad._closeValue(thisInst); //Agregado 
			...
			filter('.keypad-enter').click(function() { $.keypad._enterValue(thisInst) }).end(). //$.keypad._selectValue(thisInst, $.keypad.EN); ->Eso es lo que estaba en tre llaves -> editado -> Ahora prevengo que se inserte un caracter(salto de linea) en el campo que tiene asignado al teclado virtual
	...
	_randomiseLayout: function(inst) {
		//codigo agregado para BEE
		var randomiseAgain = this._get(inst, 'randomiseAgain');
		if (!this.firstTimeRandom && !randomiseAgain)
			return this.layoutRandomised;
		//fin codigo agregado para BEE
		...
		//codigo agregado para BEE
		if (this.firstTimeRandom){ //para guardar la primera randomisacion del layout
			this.firstTimeRandom = false;
			this.layoutRandomised = newLayout;
		}
		//fin codigo agregado para BEE
		return newLayout;
	},
	...
	isAlphabetic: function(ch) {
		return (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') || (ch == ENIE );
	},
