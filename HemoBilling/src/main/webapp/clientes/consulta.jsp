...
<script type="text/javascript">
	...
	function mostrarEstadoFiltro() {
		var mensaje = "Mostrando: C&oacute;digo: "
				...
				+ "${filtroDTO.nombre}"
				+ " | C&oacute;digo Contable: "	+ "${filtroDTO.codigoContable}"
				+ " | CUIT: "	+ "${filtroDTO.cuit}"
	...
	function refinarConsulta(confirma) {
		...
		if (confirma == "true") {
			...
			if ($('#codigoContableID').val() == "")
				$('#codigoContableID').val("Todos");
			if ($('#cuitID').val() == "")
				$('#cuitID').val("Todos");
			if ($('#localidadID').val() == "")
...
<div class="center_content">
	...
	<table style="width: 100%;">
		<tr>
			...
			<td><a href="nuevoCliente.htm" class="bt_green"> <span
	...
	<table id="rounded-corner" summary="Clientes">
		<thead>
			<tr>
				...
				<th class="rounded" scope="col"	style="text-align: center;">C&oacute;digo Contable</th>
		...
		<tbody>
			<c:forEach items="${clientesDTO}" var="cliente" varStatus="i">
				<tr>
					<td align="center">${cliente.codigo}</td>
					<td align="center">${cliente.nombre}</td>
					<td align="center">${cliente.codigoContable}</td>
					<td align="center">${cliente.cuit}</td>
					<td align="center">${cliente.telefono}</td>
					<td align="center">${cliente.email}</td>
					<td align="center"><a href="#"	onclick="editar('${cliente.codigo}')"><img src="images/user_edit.png" alt="" title="" border="0" /></a></td>
					<td align="center"><a href="#" class="askDelete"	id="${i.count}"><img src="images/trash.png" alt="" title=""	border="0" /></a> <input type="hidden" id="codigo${i.count}" value="${cliente.codigo}" /> </td>
...
<!-- FORMULARIOS DE EDICION Y ELIMINACIÓN ---- --> 
	...
	<form:form method="post" name="editarForm" id="editarForm"
		action="editarCliente.htm" commandName="clienteDTO">
		<form:hidden path="codigo" id="codigoEditarID" />
	</form:form>

	<form:form method="post" name="eliminarForm" id="eliminarForm"
		action="doEliminarCliente.htm" commandName="clienteDTO">
...
<div id="filtroBusqueda" style="display:none;" class="jqmWindow">
		<form:form  method="post" name="filtrarForm" id="filtrarForm" cssClass="niceform" action="refreshClientes.htm" commandName="filtroDTO">
			...
			<fieldset>
				...
				<dl>
					<dt> <label for="codigoRNOS">Codigo Contable: </label> </dt>
					<dd> <form:input path="codigoContable" id="codigoContableID"/> </dd>
				</dl>
		...
		<form:form  method="post" name="paginarForm" id="paginarForm" action="refreshClientes.htm" commandName="filtroPaginadoDTO">
