...
public class HTMLEntities {
	...
	static {
		 ...
		 */
		final Object[][] html_entities_table = {
			...
			// Bug minor conocido: no se pueden utilizar ampersand excepto en codificaciones html como por ejemplo los acentos
    ...
    public static String htmlentities(String str) {
        return htmlentities(str, false);
    }
	
	/**
	 * Convert special and extended characters into HTML entitities.
	 * @param str input string
	 * @param treatAllAsExtChars Add semicolon char to the end of the string.
	 * @return formatted string
	 * @see #unhtmlentities(String)
	 */
	public static String htmlentities(String str, boolean treatAllAsExtChars) {
		...
		for (int i = 0; i < str.length(); ++i) {
			...
			final String entity = htmlentities_map.get(intCh); //get equivalent html entity
			if (entity == null) { //if entity has not been found
				if (intCh > 128) { //check if is an extended character
					buf.append("&#" + intCh + ";"); //convert extended character
				} else {
					buf.append(ch); //append the character as is
				}
			} else {
			    //append the html entity
			    if (treatAllAsExtChars) {
			    	buf.append(entity + ";");
			    } else {
			    	buf.append(entity); 
			    }
	...
	public static String unhtmlentities_custom(String str) {
		...
		for (int i = 0; i < str.length(); ++i) {
			...
			if (ch == '&') {
				...
				String entity = str.substring(i, semi);
				Integer iso;
				if (entity.charAt(1) == ' ') {
					buf.append(ch);
					continue;
